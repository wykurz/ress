...
pub(crate) trait Analysis: Send + 'static {
    ...
    /// `Ok(Step::Done(()))` = the whole analysis is finished; `Ok(Step::More(..))` = call again,
    /// carrying the witness pair that it is honestly resumable (restructure R6,
    /// `crate::scan::Step`'s own doc comment) -- upgraded from a bare `Ok(bool)` specifically
    /// because that shape was `SweepAnalysis`'s own "`Ok(false)` was two livelocks' home"
    /// (`structural-scan.md` §4.1): a zero-progress `Ok(false)` here is now unconstructible the
    /// same way it already is for every scan struct's own `step`.
    /// `Err` = this unit failed (the driver retries with backoff).
    fn step(
        &mut self,
        tx: &tokio::sync::watch::Sender<Self::Snapshot>,
    ) -> impl std::future::Future<Output = anyhow::Result<crate::scan::Step<()>>> + Send;
...
mod tests {
    ...
    struct ScriptedAnalysis {
        ...
        // restructure R6: a scripted `Ok(false)` must still produce a REAL `Step::More` witness
        // pair (`Progressed`/`Charged` are both unconstructible outside their own modules -- no
        // test double gets a back door) -- no test currently scripts `Ok(false)` (this module's
        // own doc comment lists every outcome the existing tests use: `Ok(true)` and `Err`), but
        // the type must still be honestly satisfiable for the arm to compile at all. `sim_cursor`
        // is a `Descending` purely for this: `Reader::skip` charges a span WITHOUT touching
        // `sim_cache` at all (its own doc comment), so no real file content is needed -- `sim_
        // cache` exists only because `Reader::new` requires a `&BlockCache` reference to construct
        // one, never because anything reads through it.
        sim_cursor: crate::progress::Descending,
        sim_meter: crate::meter::Meter,
        sim_cache: crate::cache::BlockCache,
    }
        ...
        fn new(script: VecDeque<(u64, anyhow::Result<bool>)>) -> (Self, ScriptedCounters) {
            ...
            (
                Self {
                    ...
                    give_up_count: 0,
                    give_up_tx,
                    // starts absurdly high (never realistically exhausted by any script's own
                    // cumulative `progress` total) and descends -- see this struct's own field
                    // doc comment for why a `Descending` cursor, not an `Ascending` one, needs no
                    // real backing bytes.
                    sim_cursor: crate::progress::Descending::new(1 << 40, 0),
                    sim_meter: crate::meter::Meter::background(usize::MAX),
                    sim_cache: crate::cache::BlockCache::new(
                        std::sync::Arc::new(crate::source::MockSource::new(bytes::Bytes::new())),
                        1,
                        1,
                    ),
                },
    ...
    impl Analysis for ScriptedAnalysis {
        ...
        async fn step(&mut self, _tx: &watch::Sender<()>) -> anyhow::Result<crate::scan::Step<()>> {
            self.step_count += 1;
            ...
            match outcome {
                Ok(true) => Ok(crate::scan::Step::Done(())),
                Ok(false) => {
                    // see this struct's own field doc comment on `sim_cursor` -- no current test
                    // reaches this arm.
                    self.sim_meter.begin_step();
                    let mut reader =
                        crate::meter::Reader::new(&self.sim_cache, &mut self.sim_meter);
                    let to = self.sim_cursor.at().saturating_sub(progress);
                    reader.skip(self.sim_cursor.at(), to).expect(
                        "a scripted (0, Ok(false)) entry claims resumable progress with none \
                         simulated -- fix the script, not this mechanism",
                    );
                    let moved = self
                        .sim_cursor
                        .lower_to(to)
                        .expect("skip's own success above already proves `to < sim_cursor.at()`");
                    Ok(crate::scan::Step::More(
                        moved,
                        self.sim_meter
                            .progress_witness()
                            .expect("the skip above just charged a nonzero span"),
                    ))
                }
                Err(e) => Err(e),
            }
