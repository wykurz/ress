...
/// of detaching it (a bare `JoinHandle`'s own `Drop` only detaches; `cancel`
/// is what actually aborts, `resolve.rs`'s own doc comment on `PendingNav`).
/// Aborting this task never wedges or duplicates the block cache's own
/// fetch it may have started: that fetch runs detached from any requester's
/// lifetime (`ress-core`'s own `docs/concurrency.md`, "The shape"), so it
/// simply keeps going and lands its bytes in the cache regardless of
/// whether this scan is still around to use them.
struct ActiveNav {
    nav: Nav,
    p: PendingNav,
}
...
mod tests {
    ...
    async fn dropping_the_pending_slot_aborts_the_scan() {
        ...
        //
        // REVISED, batch 4 (2026-07-24), finding #11: `ress-core`'s own block cache became a
        // detached publisher (`BlockCache::get`'s own doc comment) -- the gated read this test
        // parks on no longer dies with the pending nav's task, so the `cancelled_events` wait
        // this test used to make (its own PRIMARY proof) would now simply never resolve. What
        // this test's own name actually claims -- the pending nav's TASK gets torn down, not the
        // read specifically -- was already, independently provable through the progress channel
        // closing (this test's own prior "direct discriminator," now promoted to the sole proof):
        // `PendingNav`'s `progress` sender lives inside its own background scan task
        // (`resolve.rs`), so it closes if and only if that task's own future is torn down or
        // returns.
        let mut data = Vec::new();
        for _ in 0..100 {
            data.extend_from_slice(b"y\n");
        ...
        // `borrow_and_update` immediately after cloning, defensively -- same idiom as
        // `ress-core/src/status.rs`'s own identical fix (which IS load-bearing there, RED-
        // verified), guarding against a fresh clone's very first `changed()` resolving against an
        // accumulated backlog rather than a genuinely new event. Corrected framing, fix round 1,
        // F8 (the report originally claimed this recurred the load-bearing case; measured, it
        // does not): here it is a no-op in practice, same as `schedule.rs`'s own sibling site --
        // structurally, not just probabilistically, since there is no `.await` between the clone
        // above and `arm_gate` below on this test's own current-thread runtime, the pending task
        // cannot even run, let alone publish, before the gate is armed, and it then parks on its
        // very first read. Kept anyway: it costs nothing and removes the coincidence.
        ...
        drop(active);
        // POSITIVE proof: the pending nav's own task was torn down, not left running or let
        // finish on its own. Bounded, not a bare `.await`: a leaked, never-aborted task would
        // leave this parked forever rather than pass silently.
        let closed = tokio::time::timeout(std::time::Duration::from_secs(5), rx.changed())
            .await
            .expect("the pending nav's progress sender never closed -- was it genuinely aborted?");
        assert!(
            closed.is_err(),
